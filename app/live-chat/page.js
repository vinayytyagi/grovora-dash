...
const messagesData = [
    { id: 1, user: { id: 1, sendTo: 2, message: "Hello, Who are you?", }, },
    { id: 2, user: { id: 2, sendTo: 1, message: "I am John, a graphic designer.", }, },
    { id: 3, user: { id: 1, sendTo: 2, message: "Nice to meet you John! I need help with some designs.", }, },
    { id: 4, user: { id: 2, sendTo: 1, message: "Sure, I'd be happy to help. What kind of designs do you need?", }, },
    { id: 5, user: { id: 3, sendTo: 4, message: "Hi Ram, how's the editing work going?", }, },
    { id: 6, user: { id: 4, sendTo: 3, message: "Hello Binod! The editing is going well, almost finished.", }, },
    { id: 7, user: { id: 3, sendTo: 4, message: "Great! When can I expect the final delivery?", }, },
    { id: 8, user: { id: 4, sendTo: 3, message: "I'll send it by tomorrow morning.", }, },
    { id: 9, user: { id: 1, sendTo: 3, message: "Hey Binod, can you help me with the development?", }, },
    { id: 10, user: { id: 3, sendTo: 1, message: "Of course Karan! What do you need help with?", }, },
    { id: 11, user: { id: 1, sendTo: 3, message: "I'm stuck with the API integration.", }, },
    { id: 12, user: { id: 3, sendTo: 1, message: "No problem, let's debug it together.", }, }
];
...
const Page = () => {
    ...
    const userData = [{
        ...
        special: "loved",
        messagesIds: [1],
        contactDetails: { email: 'karan@gmail.com', phone: '+91 1234567890', adress: "Maratha collage Madhya pradesh, India", joinDate: "12/12/2022" }
    }, {
        image: '/user.svg',
        name: 'Jone doe',
        ...
        special: "favroite",
        messagesIds: [1],
        contactDetails: { email: 'john@gmail.com', phone: '+91 0987654321', adress: "Borivali Mumbai, India", joinDate: "12/12/2024" }
    }, {
        image: '/user.svg',
        name: "binod",
        quate: "I am a developer",
        ...
        special: "cool",
        messagesIds: [1],
        contactDetails: { email: 'binod@gmail.com', phone: '+91 1234567890', adress: "Maratha collage Madhya pradesh, India", joinDate: "12/12/2022" }
    }, {
        image: '/user.svg',
        name: "Ram krishna",
        quate: "I am a Editor",
            ...
            <div className="flex-1 lg:ml-64">
                ...
                <div className="flex flex-col lg:flex-row w-full h-[91vh] overflow-hidden">
                    ...
                    <div className="w-full lg:w-2/4 bg-gray-100 h-[92vh] flex flex-col relative pb-[5%]">
                        <div className="lg:hidden bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
                            <button onClick={() => setShowChatList(true)} className="p-2 rounded-lg text-gray-600 hover:bg-gray-100">
                            ...
                            <button onClick={() => setShowUserDetails(true)} className="p-2 rounded-lg text-gray-600 hover:bg-gray-100">
                        ...
                        <div style={{ backgroundImage: "url('/assets/ChatBackground.svg')" }} className="messages flex flex-col gap-4 px-4 pt-8 scrollbar-hide pb-24 flex-1 overflow-y-auto">
                            {messages
                                .filter(message =>
                                    (message.user.id === selectedChat && message.user.sendTo === agent.id) ||
                                    (message.user.id === agent.id && message.user.sendTo === selectedChat)
                                )
                                .sort((a, b) => new Date(a.timestamp || 0) - new Date(b.timestamp || 0))
                                .map((message) => {
                                    const user = userData.find((user) => user.userId === message.user.id);
                                    const isSender = agent.id === user.userId;
                                    const messageTime = message.timestamp ? new Date(message.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '';
                                            ...
                                            {isSender ? (
                                                <div className="flex justify-end gap-2 relative mt-2">
                                                    <div className="flex flex-col max-w-[70%] gap-1">
                                                        <p className="bg-green-500 text-white rounded-lg rounded-tr-none px-4 py-2 w-full break-words shadow-sm">
                                                            {message.user.message}
                                                        </p>
                                                        <span className="text-xs text-[#618A7D] self-end mr-2">{messageTime}</span>
                                                    </div>
                                                ...
                                                <div className="flex justify-start relative gap-2 mt-2">
                                                    ...
                                                    <div className="flex flex-col gap-1">
                                                        ...
                                                        <span className="text-xs text-[#618A7D] ml-2">{messageTime}</span>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                ...
                {showChatList && (
                    <>
                        <div className="lg:hidden fixed inset-0 bg-black bg-opacity-50 z-40" onClick={() => setShowChatList(false)} />
                        <div className="lg:hidden fixed inset-y-0 left-0 w-64 bg-white z-50 overflow-y-auto">
                            <div className="p-4 border-b border-gray-200 flex items-center justify-between">
                                <h2 className="text-lg font-bold">Chats</h2>
                                <button onClick={() => setShowChatList(false)} className="p-2 rounded-lg text-gray-600 hover:bg-gray-100">
                ...
                {showUserDetails && (
                    <>
                        <div className="lg:hidden fixed inset-0 bg-black bg-opacity-50 z-40" onClick={() => setShowUserDetails(false)} />
                        <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white z-50 rounded-t-xl overflow-y-auto max-h-[70vh]">
                            <div className="p-4 border-b border-gray-200 flex items-center justify-between">
                                <h2 className="text-lg font-bold">Customer Details</h2>
                                <button onClick={() => setShowUserDetails(false)} className="p-2 rounded-lg text-gray-600 hover:bg-gray-100">
                                    <span className="text-xl">×</span>
                                </button>
                            </div>
